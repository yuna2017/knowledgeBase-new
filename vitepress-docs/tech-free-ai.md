---
description: 免费 AI API 额度汇总：先说明中国大陆在账号、付款和条款上的限制，再分海外与国内平台列出可用额度，并给出选择平台时的核对项和编程调用的基本做法。额度随时调整，以各平台官方页面为准。
tags:
  - AI工具
  - 免费资源
  - API
...
OpenAI、Anthropic 和 Google Gemini API 的官方支持地区目前均不包含中国大陆，能连上不代表账号注册、付款方式或使用方式符合平台条款。中转站只改变请求经过的线路，不能消除账号、隐私、资金、模型真实性和数据跨境风险，哪些材料不要发、用前要核对什么见 [API 中转站简介](./tech-relay.md)。
