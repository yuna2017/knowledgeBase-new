...
- 代码注释全部使用中文，方便理解
- 模型、价格、安装命令和第三方工具信息变化很快；本文中相关信息是在 **2026-05-18** 按官方文档做过一次核对的，正式使用前仍应以官网当前页面为准
...
想象这样一个场景：南完全不会写代码，但他用自然语言向AI描述了自己想要的功能 —— "帮我做一个网协内部的知识库工具，支持按标签分类和搜索"。三天后，一个功能完整、界面美观的Web应用上线了。
...
-  不需要精通任何编程语言（Python、JavaScript、Java……统统不需要）
-  不需要太高的数学基础
-  不一定是CS相关专业（像南那样）
-  不熟悉任何开发工具
...
| 掌握核心工具 | 能熟练使用Claude Code和（或）Codex完成完整的软件开发 |
| 灵活选模型 | 能根据任务需求选择最合适的AI模型（Claude/GPT/DeepSeek/GLM等） |
...
# 在你喜欢的位置创建一个总的工作目录
# Windows 建议：
$ mkdir D:\vibe-coding-projects

# macOS 建议：
$ mkdir ~/vibe-coding-projects

# 进入这个目录
$ cd D:\vibe-coding-projects  # Windows
$ cd ~/vibe-coding-projects   # macOS
...
预期输出：
```
Initialized empty Git repository in .../vibe-coding-projects/.git/
```
...
> **提示**：氛围编程不是"不需要懂任何技术"，而是大幅降低了入门门槛。随着你的使用越来越深入，你会自然而然地积累技术知识。这个过程是"边用边学"，而非传统的"先学后用"。
...
**第三阶段：智能体编程时代（2024 至今）** ← 我们现在就在这里（本文完成于2026年6月底）
...
AI不是像人一样一个字一个字地读代码，它把文本切成一个个小块，叫做 **Token（词元）**。
...
- **代码质量不可控**：AI可能写出能跑但很乱的代码，积累成"技术债务"（屎山代码这一块）
...
你有没有过这种经历：在淘宝买衣服时，你描述"好看的衣服"，结果搜到的大部分无法符合你的预期？这就是**沟通不精准**导致的。
...
```
Given（前提条件）：系统中有10科考试成绩，其中4科标记了"高等数学"标签
When（操作）：用户点击"高等数学"标签
Then（预期结果）：页面只显示这4科标记了"高等数学"标签的考试成绩
```
...
### 1.4 主流模型系列介绍（更新至2026年5月中旬，请以各家模型官网最新信息为准）
...
Claude 是本教程推荐的主力模型，由 Anthropic 公司（俗称A社）开发。
...
| 使用强度            | 推荐模型                  | 月费用估算                |
| ------------------- | ------------------------- | ------------------------- |
| 轻度（每天1-2小时） | DeepSeek API / GLM / Kimi | ¥5-30（视模型与套餐计算） |
...
> **注意**：使用云端AI服务时，你发送的代码**会经过服务商的服务器（甚至其他不必要的文件也会一并打包发送给服务商）**。虽然主流服务商（Anthropic、OpenAI等）承诺“不会”使用用户数据训练模型**（已成笑话）**，但如果你在处理高度敏感的代码（如核心算法、密钥等），建议使用本地部署方案。
...
L4 项目管理 ─── 多Agent协同、任务分解（Qoder）
   ↑
L3 任务执行 ─── 自主修改文件、运行命令（Claude Code、Codex）我们重点学这个
   ↑
...
本教程的核心工具 Claude Code 处于 **L3 层级**，是目前日常开发中实用且强大的层级。
...
##### 2.2.1.1 LLM Loop：Claude Code（简称cc） 凭什么是「Agent」而不是「聊天框」
...
-  你可以给 cc 一个**模糊的高层目标**（如做具体的软件），它会自己一步步推进
...
| 形态 | 说明 | 适合人群 |
|------|------|---------|
| **Codex Desktop（2026年7月注：Codex现已和ChatGPT合并）** | 图形界面桌面客户端，体验最好 | 新手、喜欢 GUI |
...
| 国内 Coding Plan | 需自行配中转（自配或CC Switch等） | 国内厂商原生支持 |

**何时选择 Codex：**

- 已有 ChatGPT 订阅或 OpenAI API → 零配置开箱即用
...
##### 2.3.2.1 方案一：使用 Anthropic 官方 API

这是最直接的配置方式，适合能稳定访问 Anthropic 服务、具备合规支付方式的用户（咳咳）。国内用户可能会遇到注册、支付、访问稳定性等问题，建议按自己的网络和合规条件谨慎选择。
...
（信息按 Anthropic 官方价格页在 2026-05-18 核对，请以官网最新价格为准）
...
选择主题1-7：直接按数字即可（一般就Auto）

确认Security notes、Accessing workspace获取目录权限等
...
浏览器会自动打开 `http://localhost:8501`。如果没自动打开，手动访问地址即可。
...
## 第七部分：Codex Desktop安装和使用教程（部分内容已过时）
...
>
> **最新注释**：Codex已于2026年7月初合并入ChatGPT桌面端内，下面部分内容（例如安装和登录）已过时，请选择性学习，如有问题，可看最新我单开的教程里是否有你想要的答案喵。
...
```text
请帮我创建一个自动化任务：每周一上午9点，读取我的项目资料文件夹，生成一份本周新增资料摘要。创建前先展示任务内容、执行频率和输出格式。
```
