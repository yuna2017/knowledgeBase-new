...
title: 仓库说明与维护指引
description: YUNA KnowledgeBase 由燕山大学大学生网络信息协会维护，内容按 tags 组织。本页说明知识库的定位、内容时效性约定、参与维护的方式，以及基于 VitePress 的本地运行与构建步骤。
---
...
- [标签索引](/tags)
- [内容术语与维护规范](/CONTEXT)
- 需求反馈这条线一共四个页面：读者入口是[告诉我们缺什么](/wanted)，提交成功后跳到[确认页](/wanted-done)，公开进度在[反馈执行状态](/wanted-status)，维护者看[反馈审计](/wanted-audit)（需口令）。
  这四个页面刻意不带 `tags`：它们是功能页不是文章，带了就会以「收到了」这种标题混进标签页的正文列表里。
