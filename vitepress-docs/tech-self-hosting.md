...
- **Cloudflare Tunnel**：不需要公网 IP，设备主动连接 Cloudflare。Tunnel 不等于私网，公开域名仍需配置 Cloudflare Access 或应用自身认证。**校园网环境下的校园网服务无法使用 Cloudflare Tunnel，中国电信等运营商服务可以使用**
...
面向游戏服务器等场景的更详细介绍（NAT 原理、穿透软件对比、申请公网 IP 实测）见 [Minecraft 服务器教程（序言）](./tech-build-your-own-minecraft-server-0.md)。
...
4. 若网站部署在中国大陆服务器并向公众开放，先确认 ICP 备案等要求（见[域名申请与管理](./tech-domain.md#避坑提醒)）。境外托管也要遵守内容和数据处理规定。
