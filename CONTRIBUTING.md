...
- 搜索引擎摘要和分享卡片的描述，默认自动取正文第一段（或第一组列表）。想自己指定时，在 frontmatter 里加一行 `description:`
- 描述长度必须在 80 到 130 字之间，构建时会检查，不合格会直接失败。按中文算，搜索结果里 80 到 90 个字就把摘要铺满了，更长的部分显示不出来
- 首段是免责声明、转载说明，或者以冒号结尾只有半句话时，自动摘要不适合当搜索结果，这类页面要手写 `description:`
...
合并到 `main` 后，部署流程会把本次改动的页面推送给 IndexNow（Bing、Yandex 等共用），
不用手动去搜索引擎后台提交。它依赖 `vitepress-docs/public/` 下的密钥文件——
文件名和文件内容必须相同，删掉或改名会让推送静默失效（部署本身不受影响）。
